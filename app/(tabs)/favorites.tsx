...
export default function WatchlistScreen() {
  ...
  const [sortBy, setSortBy] = useState<'endTime' | 'currentBid' | 'addedAt'>(
    'endTime'
  );
  ...
  const setPriceAlert = (itemId: string, alertPrice: number) => {
    ...
    Alert.alert(
      'Price Alert Set',
      `You'll be notified when the bid reaches ${alertPrice.toLocaleString()} Connects`
    );
  };
  ...
  return (
    <SafeAreaView style={[styles.container]}>
      ...
      <View style={styles.statsContainer}>
        ...
        <LinearGradient colors={['#2196F3', '#1976D2']} style={styles.statCard}>
          ...
          <Text style={styles.statNumber}>
            {
              watchlist.filter(
                (item) => item.endTime.getTime() - Date.now() < 60 * 60 * 1000
              ).length
            }
          </Text>
      ...
      <View style={styles.filtersContainer}>
        ...
        >
          <TouchableOpacity
            style={[styles.filterChip, !filterBy && styles.filterChipActive]}
          ...
          {categories.map((category) => (
            <TouchableOpacity
              ...
              onPress={() =>
                setFilterBy(filterBy === category ? null : category)
              }
            >
              <Text
                style={[
        ...
        {filteredWatchlist.length > 0 ? (
          ...
          <AnimatedCard style={styles.emptyState}>
            ...
            <Text style={styles.emptySubtitle}>
              Start adding items to your watchlist by tapping the heart icon on
              any auction
            </Text>
            <TouchableOpacity
...
const styles = StyleSheet.create({
  ...
  statsContainer: {
    ...
    paddingVertical: 20,
    gap: 12, // Use gap for spacing between stat cards
  },
  statCard: {
    flex: 1,
    alignItems: 'center',
  ...
  filtersContainer: {
    ...
    borderBottomWidth: 1,
    borderBottomColor: '#E5E7EB',
    marginBottom: 16, // Add margin below filters container
  },
  filtersScroll: {
    paddingHorizontal: 16, // Adjusted horizontal padding for scroll content
    gap: 8, // Gap between filter chips
  },
  filterChip: {
  ...
  sortButton: {
    padding: 12,
    marginRight: 16, // Consistent right margin
  },
  content: {
    flexGrow: 1, // Use flexGrow instead of flex: 1 for ScrollView content
  },
  watchlistGrid: {
    paddingHorizontal: 16, // Adjusted horizontal padding
    paddingBottom: 20, // Add bottom padding to the grid itself
    gap: 16,
  },
  watchlistCard: {
    backgroundColor: '#FFFFFF',
  ...
  cardImageContainer: {
    height: 180, // Slightly reduced image height
    position: 'relative',
  },
  cardImage: {
    width: '100%',
  ...
  cardCategory: {
    fontSize: 13, // Slightly smaller category font
  ...
  emptyState: {
    alignItems: 'center',
    padding: 30, // Reduced padding for empty state
  ...
  bottomPadding: {
    height: 80, // Slightly reduced bottom padding
  },
});
